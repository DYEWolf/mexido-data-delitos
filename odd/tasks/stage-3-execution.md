...
- [x] S3-09 — Mapas con valor para los 125 municipios (local, sin desplegar, 2026-09-25). Las piezas 1, 5 y 11 exportan su tabla municipal completa (resultados previos idénticos, mismos sha256 de insumos). Tasas: color por la tasa suavizada respecto del estado y borde para diferencias creíbles; pieza 1: qué pesa más, desaparición u homicidio; tendencias: color fuerte = cambio creíble, claro = estimación sin certeza (el modelo jerárquico acerca a la tendencia típica, no a cero). Tablas y fichas con los 125 municipios. Módulo compartido `src/worker/layers.mjs`.
